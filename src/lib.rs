...
pub fn calculate_witness(input_json: &str, graph_bytes: &[u8]) -> eyre::Result<Vec<U256>> {
    let (nodes, signals): (Vec<Node>, Vec<usize>) = postcard::from_bytes(graph_bytes)?;
    ...
    let v: Value = serde_json::from_str(input_json).unwrap();
        ...
        for (key, value) in map {
            ...
            if value.is_array() {
                for (idx, item) in value.as_array().unwrap().iter().enumerate() {
                    ...
                    set_input_signal_eval(input_map.clone(), &mut inputs, h, idx as u64, x);
                }
            } else {
                let x = U256::from_str(value.as_str().unwrap()).unwrap();
                set_input_signal_eval(input_map.clone(), &mut inputs, h, 0, x);
            }
        }
    }
...
pub fn build_witness() -> eyre::Result<()> {
    ...
    let total_input_len = (ffi::get_main_input_signal_no() + ffi::get_main_input_signal_start()) as usize;
